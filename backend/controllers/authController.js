...
dotenv.config();
  ...
  export const signup = async (req, res) => {
    ...
    try {
      ...
      if (existingUser) {
        validationErrors.push({ field: "email", message: "Email already registered." });
      }
      ...
      await newUser.save();
      if(!newUser.isAdmin) {
        const newGameRecord = new GameStatus({ user_id: newUser._id, unlocked_songs: [], unlocked_instruments: [], highscore: [] });
